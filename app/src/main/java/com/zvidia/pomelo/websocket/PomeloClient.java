...
public class PomeloClient extends WebSocketClient {
    public static final String HANDSHAKE_RES_HOST_KEY = "127.0.0.1";
    public static final String HANDSHAKE_RES_PORT_KEY = "3014";
    ...
    private Map<String, OnDataHandler> pushDataHandlerMap = new HashMap<String, OnDataHandler>();
    ...
    /**
     * 新增接收推送方法
     * @param route 接收推送route
     * @param onDataHandler 接收回调
     */
    public void on(String route, OnDataHandler onDataHandler) {
        pushDataHandlerMap.put(route, onDataHandler);
    }
    ...
    private void onData(PomeloPackage.Package decode) throws PomeloException, JSONException {
        ...
        if (id > 0) {
            ...
            OnDataHandler onDataHandler = pushDataHandlerMap.get(message.getRoute());
            if (onDataHandler != null) {
                onDataHandler.onData(message);
            } else {
                System.out.println("can not find onDataHander for msg:" + message.toString());
